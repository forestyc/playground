...
func (l *ListPage) Run() {
	...
	if err = l.page[0].crawler.Run(); err != nil {
		l.ctx.Logger.Error("[媒体聚焦]爬取列表页失败", zap.Error(err), zap.String("url", l.page[0].crawler.Url))
		return
	}
	l.ctx.Logger.Info("[媒体聚焦]爬取列表页成功", zap.String("url", l.page[0].crawler.Url))
	for i := 1; i <= l.totalPage; i++ {
		page := &PageInfo{ctx: l.ctx}
		url := fmt.Sprintf(_const.FocusPageUrlFormat, i)
		...
		l.ctx.Logger.Info("[媒体聚焦]爬取列表页成功", zap.String("url", page.crawler.Url))
	...
	return func() {
		...
		p.crawler.Crawler.OnHTML(selector, func(e *colly.HTMLElement) {
			if !strings.HasPrefix(e.Text, _const.PageTotalPrefix) ||
				...
				errMsg := `[媒体聚焦]总页码文本格式错误，应为"` + _const.PageTotalPrefix + `x` + _const.PageTotalSuffix + `", 实际为"` + e.Text + `"`
				p.ctx.Logger.Error("[媒体聚焦]爬取列表页失败", zap.String("url", errMsg))
				return
			}
...
func (p *PageInfo) err() crawler.Callback {
	return func() {
		p.crawler.Crawler.OnError(func(r *colly.Response, e error) {
			p.ctx.Logger.Error("[媒体聚焦]爬取列表页失败", zap.Error(e), zap.Any("rsp", r))
		})
	}
}

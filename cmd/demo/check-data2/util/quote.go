...
// GetSeriesFromContract 截取合约id中的合约系列
func GetSeriesFromContract(contract string) string {
	reg, _ := regexp.Compile(`^[a-zA-Z]+\d+`)
	if reg.Match([]byte(contract)) {
		matches := reg.FindAllStringIndex(contract, -1)
		return contract[0:matches[0][1]]
	}
	return ""
}

...
// Init 初始化
func (gn *GfexNews) Init(ctx context.GlobalContext, config zap.Config) {
	gn.ctx = ctx
	gn.ctx.C.Log = config
	// 初始化日志
...
func (gn *GfexNews) SaveArticle(articles []*Article) {
	...
	if int(session.RowsAffected) != len(articles) {
		gn.ctx.Logger.Warn("[本所要闻]文章页保存失败")
	}
	session.Commit()
	gn.ctx.Logger.Info("[本所要闻]文章保存结束", rawZap.Int64("RowsAffected", session.RowsAffected))
}
